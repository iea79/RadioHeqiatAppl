...
const Book = ({token, userId, userFavorites, data, navigation}) => {
  ...
  const toggleFavorite = async () => {
    await authService
      .addToFavoritList(userId, data.id, token)
      .then(resp => {
        console.log(resp);
    ...
    <View style={row}>
        ...
        <View style={cover}>
          {data?.cover ? (
            ...
            <Image
              resizeMode={'cover'}
              source={{
                uri: 'https://radioheqiat.fm/wp-content/uploads/logo-footer.png',
              }}
              style={img}></Image>
          )}
        </View>
      ...
      <Pressable
        ...
        <Image
          resizeMode={'contain'}
          source={
            favor ? require('./heart-active.png') : require('./heart.png')
          }
          style={favorite.img}></Image>
      </Pressable>
    </View>
  );
};
