...
const defaultCover = require('../../assets/img/book.png');
...
const bgGrade = `<svg width="100%" height="100%" viewBox="0 0 600 450" fill="none" preserveAspectRatio="xMinYMin meet" xmlns="http://www.w3.org/2000/svg">
...
const BookScreen = ({navigation, route}) => {
  ...
  const {userFavorites, userId, token} = useSelector(state => state);
  const {id, image, cover, authorName, authorRead, title, content, audio} =
    route.params.data;
  ...
  const [coverBook, setCoverBook] = useState(defaultCover);
  ...
  useEffect(() => {
    if (cover) {
      setCoverBook({src: cover});
    } else {
      setCoverBook(defaultCover);
    }
  }, [cover]);
    ...
    if (!player && audio) {
      const sound = new Sound(audio, '', error => {
        ...
        const playerTime = () => {
          ...
          return msc <= 3600000
            ? `${msc.getMinutes()}min ${msc.getSeconds()}sec`
            : `${msc.getHours()}h ${msc.getMinutes()}min ${msc.getSeconds()}sec`;
        };
  ...
  return (
    <ScrollView>
      <StatusBar backgroundColor="#82278A" />
      <View style={styles.wrapper}>
        <View style={styles.top}>
          <View style={styles.cover}>
            ...
            {image && !svgError ? (
              ...
              <Image style={styles.cover.img} source={coverBook} />
            )}
          </View>
          <View style={styles.head}>
            ...
            <Pressable
              ...
              <Image
                ...
                source={
                  favor
                    ? require('../../assets/img/heart-active.png')
          ...
          <View style={(styles.autor, styles.top.author)}>
            <Text style={styles.autor.label}>Հեղինակ:</Text>
        ...
        <View style={styles.text}>
          <RenderHTML
            tagsStyles={styles.text.html}
            contentWidth={width}
            source={{html: content}}
          />
        </View>
        {authorRead ? (
          <View stylereact-native-linear-gradient={(styles.autor, styles.row)}>
...
const styles = StyleSheet.create({
  ...
  cover: {
    ...
    bg: {
      ...
      height: '100%',
      //   objectFit: 'cover',
      //   background: linearGradient(107.43deg, #82278A 1.96%, #6D2BD7 97.51%)
    },
  },
  autor: {
    fontSize: 12,
