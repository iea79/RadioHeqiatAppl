...
const Favorite = ({userId, token, userFavorites, navigation, route}) => {
      ...
      <View style={{paddingBottom: 105}}>
        <View style={styles.tabs}>
          <Pressable
            ...
            <Text style={favorite ? styles.tab.nameCurrent : styles.tab.name}>
              Պատմություն
            </Text>
          </Pressable>
          <Pressable
            ...
            <Text style={history ? styles.tab.nameCurrent : styles.tab.name}>
              Ֆավորիտներ
            </Text>
          </Pressable>
        </View>
        <FlatList
          ...
          style={styles.list}
          renderItem={({item}) => {
            return <Book data={item} navigation={navigation} />;
          }}
        />
